...
protected:
    ...
    // Вспомогательная функция для экранирования SQL-значений
    std::string escape(const std::string& value) const {
        std::string escaped = value;
        // Простое экранирование кавычек для примера
        size_t pos = 0;
...
public:
    ...
    static std::vector<std::shared_ptr<Derived>> where(const std::string& condition) {
        ...
        try {
            ...
            // Предполагаем, что Database::query возвращает вектор map'ов
